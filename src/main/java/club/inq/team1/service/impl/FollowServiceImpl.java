...
public class FollowServiceImpl {
    ...
    public ResponseEntity<?> follow(Long currentUserId, Long opponentId) {
        ...
        if (followRepository.findByFollowerIdAndFolloweeId(currentUser.get(), opponentUser.get()).isPresent()) {
    ...
    public ResponseEntity<?> unfollow(Long currentUserId, Long opponentId) {
        ...
        Optional<User> currentUser = userRepository.findById(currentUserId);
        Optional<User> opponentUser = userRepository.findById(opponentId);
        Optional<Follow> follow = followRepository.findByFollowerIdAndFolloweeId(currentUser.get(), opponentUser.get());
        if (follow.isPresent()) {
    ...
    // 팔로워 조회 (전체 팔로워 목록)
    public List<Long> findAllFollowers(Long currentUserId) {
        Optional<User> currentUser = userRepository.findById(currentUserId);
    ...
    public Optional<Follow> findSpecificFollower(Long currentUserId, Long opponentId) {
        Optional<User> currentUser = userRepository.findById(currentUserId);
        Optional<User> opponentUser = userRepository.findById(opponentId);
    ...
    // 팔로윙 조회 (전체 팔로윙 목록)
    public List<Long> findAllFollowees(Long currentUserId) {
        Optional<User> currentUser = userRepository.findById(currentUserId);
        if (currentUser.isPresent()) {
    ...
    public Optional<Follow> findSpecificFollowee(Long currentUserId, Long opponentId) {
        Optional<User> currentUser = userRepository.findById(currentUserId);
        Optional<User> opponentUser = userRepository.findById(opponentId);
        if (currentUser.isPresent() && opponentUser.isPresent()) {
